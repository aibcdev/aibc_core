...
- `backend/src/services/llmService.ts`:
  - Changed default model from `gemini-2.5-flash` to `gemini-2.0-flash`
  - Updated `getProviderForTier` to use `gemini-2-flash` (2.0-flash)
...
- **gemini-1.5-pro**: Paid, but higher quality
- **DeepSeek**: Very cheap, no daily limit

### Recommendation
**Use `gemini-2.0-flash`** - best balance of:
...
- Could cause blank screen during session check
...
### ✅ Fixed
1. **API Quota**: Switched to `gemini-2.0-flash` (250/day vs 20/day)
2. **Blank Screen**: Added loading state and proper view handling
3. **Model Selection**: Using better free-tier model
...
1. Check backend logs - should see: `Using Gemini 2.0 Flash`
2. Test sign-in - should show loading then content (no blank)
3. Run multiple scans - should not hit quota
