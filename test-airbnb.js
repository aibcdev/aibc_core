/**
 * Critical Test: Airbnb.com Social Media Scraping
...
async function testAirbnb() {
  console.log('='.repeat(80));
  console.log('CRITICAL TEST: Airbnb.com Social Media Scraping');
  console.log('='.repeat(80));
  console.log('\n🎯 Test Objective:');
  console.log('   Verify that the scanner finds ALL social media links from airbnb.com');
  console.log('   and scrapes each platform independently without skipping any.');
  console.log('\n📋 Expected Social Links on Airbnb.com:');
  console.log('   - Twitter/X: @Airbnb');
  console.log('   - Instagram: @airbnb');
  console.log('   - LinkedIn: /company/airbnb');
  console.log('   - Facebook: /airbnb');
  console.log('   - YouTube: @airbnb');
  console.log('   - TikTok: @airbnb');
  console.log('\n🚀 Starting scan...\n');
  
  try {
    // Start scan with all major platforms
    const platforms = ['twitter', 'instagram', 'linkedin', 'youtube', 'tiktok', 'facebook'];
    const startResponse = await startScan('airbnb.com', platforms, 'standard');
    const scanId = startResponse.scanId;
    
    console.log(`📝 Scan ID: ${scanId}`);
    console.log(`🎯 Target: airbnb.com`);
    ...
    console.log('\n' + '='.repeat(80));
    console.log('🔍 CRITICAL REVIEW - Airbnb.com Scan Results');
    console.log('='.repeat(80));
    ...
    if (issues.some(i => i.includes('CRITICAL'))) {
      ...
      console.log('   The scanner should find social links that are clearly visible on airbnb.com');
    ...
    console.log('\n💾 Detailed results saved to airbnb-test-results.json');
...
async function checkBackend() {
  try {
    ...
    // We expect an error, but if we get a response, backend is running
    return true;
  } catch (error) {
