...
export class PromptGenerator {
     ...
     */
    buildCharacterContext(snapshot) {
...
You must remain consistent with this persona at all times.`;
    ...
    buildBrandOverlay(overlay) {
        ...
        if (warmth > 0)
            toneDescriptions.push('Warmer, more personable');
        if (warmth < 0)
            toneDescriptions.push('Slightly less warm, more professional distance');
        if (confidence > 0)
            toneDescriptions.push('More authoritative and confident');
        if (confidence < 0)
            toneDescriptions.push('More measured and humble');
        if (formality > 0)
            toneDescriptions.push('More formal and professional');
        if (formality < 0)
            toneDescriptions.push('More casual and approachable');
...
- CTA style: ${style_constraints.cta_style.replace('_', ' ')}`;
        ...
        if (compliance_rules.no_false_claims)
            complianceLines.push('- Do not make unsupported claims');
        if (compliance_rules.no_financial_promises)
            complianceLines.push('- Do not make financial guarantees or projections');
...
${styleSection}${complianceSection}`;
    ...
    buildTaskPrompt(task) {
        ...
        // Format deliverables
        const deliverables = outputs_required.map((o, i) => `${i + 1}. ${o.charAt(0).toUpperCase() + o.slice(1).replace(/_/g, ' ')}`).join('\n');
        // Format constraints
        const constraintLines = [];
        if (constraints.max_words)
            constraintLines.push(`- Maximum ${constraints.max_words} words`);
        if (constraints.format)
            constraintLines.push(`- Format: ${constraints.format.replace(/_/g, ' ')}`);
...
${deliverables}${constraintsSection}`;
     ...
     */
    composeSingle(snapshot, overlay, task) {
        return {
