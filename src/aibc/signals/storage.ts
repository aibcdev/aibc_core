 ...
 */
export async function saveExecutiveBrief(brandId: string, brief: AgentOutput) {
    const client = getSupabaseClient();
    if (!client) return;
    ...
    if (error) console.error('Error saving executive brief:', error);
}

/**
 * Fetch chat history for an agent
 */
export async function fetchChatHistory(agentType: AgentType) {
    const client = getSupabaseClient();
    if (!client) return [];
 ...
 * Save a chat message
 */
export async function saveChatMessage(agentType: AgentType, role: 'user' | 'agent', content: string) {
    const client = getSupabaseClient();
    if (!client) return;
