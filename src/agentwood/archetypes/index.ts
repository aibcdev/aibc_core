...
export function findArchetypesByDomain(domain: string): ArchetypeDefinition[] {
    return listArchetypes().filter(a =>
        a.recommended_domains.some(d => d.toLowerCase().includes(domain.toLowerCase()))
    );
}
