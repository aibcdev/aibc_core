...
export async function runSignalPipeline(
    ...
    if (config.newsApiKey && config.newsQueries) {
        for (const query of config.newsQueries) {
            try {
                // @ts-ignore - Argument mismatch fix pending
                const newsSignals = await fetchNewsSignals(query);
    ...
    let executiveBrief: AgentOutput | null = null;
    if (outputs.length > 0) {
        try {
 ...
 * Quick test function
 */
export async function testPipeline(geminiApiKey: string): Promise<void> {
