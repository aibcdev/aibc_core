/**
 * Critical Test: Surge.xyz Social Media Scraping
...
async function testSurge() {
  console.log('='.repeat(80));
  console.log('CRITICAL TEST: Surge.xyz Social Media Scraping');
  console.log('='.repeat(80));
  console.log('\n🎯 Test Objective:');
  console.log('   Verify that the scanner finds ALL social media links from surge.xyz');
  console.log('   and scrapes each platform independently without skipping any.');
  console.log('\n📋 Expected Social Links on Surge.xyz:');
  console.log('   - Twitter/X: @surge_xyz or similar');
  console.log('   - Instagram: @surge.xyz or similar');
  console.log('   - LinkedIn: /company/surge or similar');
  console.log('   - YouTube: @surge or similar');
  console.log('   - TikTok: @surge or similar');
  console.log('   - GitHub: /surge-xyz or similar (common for tech companies)');
  console.log('\n🚀 Starting scan...\n');
  
  try {
    // Start scan with all major platforms
    const platforms = ['twitter', 'instagram', 'linkedin', 'youtube', 'tiktok', 'facebook', 'github'];
    const startResponse = await startScan('surge.xyz', platforms, 'standard');
    const scanId = startResponse.scanId;
    
    console.log(`📝 Scan ID: ${scanId}`);
    console.log(`🎯 Target: surge.xyz`);
    ...
    console.log('\n' + '='.repeat(80));
    console.log('🔍 CRITICAL REVIEW - Surge.xyz Scan Results');
    console.log('='.repeat(80));
    ...
    if (results.socialLinks) {
      ...
      const expectedPlatforms = ['twitter', 'instagram', 'linkedin', 'youtube', 'github'];
      const foundPlatforms = foundLinks.filter(p => expectedPlatforms.includes(p));
    ...
    if (!results.socialLinks || Object.keys(results.socialLinks).length === 0) {
      ...
      if (foundCount < 2) {
        issues.push(`❌ CRITICAL: Only ${foundCount} social links found (expected 3+)`);
      } else if (foundCount < 3) {
        issues.push(`⚠️  WEAK: Only ${foundCount} social links found (expected 3+)`);
      } else if (foundCount < 5) {
        issues.push(`⚠️  PARTIAL: ${foundCount} social links found (expected 5+)`);
      } else {
    ...
    if (issues.some(i => i.includes('CRITICAL'))) {
      ...
      console.log('   The scanner should find social links that are clearly visible on surge.xyz');
    ...
    console.log('\n💾 Detailed results saved to surge-test-results.json');
