        ...
        <div className="bg-[#0A0A0A] border border-white/10 rounded-xl p-6">
          ...
          <div className="flex items-baseline gap-2 mb-3">
            <span className="text-3xl font-bold text-white">
              {loading ? '...' : analyticsData?.marketShare?.yourShare?.toFixed(1) || '0.0'}%
            </span>
            <span className="text-sm font-bold text-green-400 flex items-center gap-1">
              <TrendingUp className="w-4 h-4" />
              {analyticsData?.engagement?.comparison?.difference > 0 ? '+' : ''}
              {analyticsData?.engagement?.comparison?.difference?.toFixed(1) || '0.0'}%
            </span>
          </div>
          <div className="h-2 bg-white/5 rounded-full overflow-hidden mb-2">
            <div 
              className="h-full bg-gradient-to-r from-green-500 to-emerald-500 rounded-full" 
              style={{ width: `${Math.min(analyticsData?.marketShare?.yourShare || 0, 100)}%` }}
            ></div>
          </div>
          <div className="text-xs text-white/60">
            Ranked #{analyticsData?.marketShare?.rank || 'N/A'} in Industry
          </div>
        </div>

        {/* Engagement Score */}
        <div className="bg-[#0A0A0A] border border-white/10 rounded-xl p-6">
          <div className="text-xs text-white/40 mb-2 uppercase tracking-wider">Engagement Score</div>
          <div className="flex items-baseline gap-2 mb-3">
            <span className="text-3xl font-bold text-white">
              {loading ? '...' : Math.round(analyticsData?.engagement?.score || 0)}
            </span>
            <span className={`text-sm font-bold flex items-center gap-1 ${
              (analyticsData?.engagement?.comparison?.difference || 0) > 0 ? 'text-green-400' : 'text-red-400'
            }`}>
              {(analyticsData?.engagement?.comparison?.difference || 0) > 0 ? <TrendingUp className="w-4 h-4" /> : <TrendingDown className="w-4 h-4" />}
              {analyticsData?.engagement?.comparison?.difference > 0 ? '+' : ''}
              {analyticsData?.engagement?.comparison?.difference?.toFixed(1) || '0.0'} vs avg
            </span>
          </div>
          <div className="flex gap-1 mb-2">
            {[1, 2, 3, 4, 5].map((i) => {
              const score = analyticsData?.engagement?.score || 0;
          ...
          <div className="text-xs text-white/60">
            Avg session: {Math.round(analyticsData?.engagement?.metrics?.avgSessionDuration / 60) || 0} min
          </div>
        </div>
